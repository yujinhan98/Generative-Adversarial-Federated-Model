...
def train_GAFM(Epochs, features, train_loader, test_loader, gamma=1, gamma_w=10, sigma=0.01, delta=0.1, lr=1e-3,
    ...
    for epoch in range(Epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            intermidiate_to_server=torch.sigmoid(intermidiate_to_server.sum(dim=1))#intermidiate_to_server.mean(dim=1)#
            loss_recon = gamma * BCE(intermidiate_to_server.view(-1, 1),
                                     Y_dot.unsqueeze(-1).type_as(intermidiate_to_server))

            loss = loss_recon + loss_D_1
