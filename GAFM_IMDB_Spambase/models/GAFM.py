...
def train_GAFM(Epochs, features, train_loader, test_loader, gamma=1, gamma_w=1, sigma=0.01, delta=0.1, lr=1e-3,
    ...
    logger.flush()
    logger.write(
        '\nGamma={:.3f}\t Gamma_w={:.3f},delta={:.3f}\t lr{:.3f},'.format(
    ...
    # opt_1 = optim.RMSprop(model_1.parameters(), lr=lr)#optim.Adam(model_1.parameters(), lr=lr)
    # opt_2 = optim.RMSprop(model_2.parameters(), lr=lr) #optim.Adam(model_2.parameters(), lr=lr)
    ...
    training_labels = []
    outputs_list = []
    # print('gamma_w',gamma_w,'gamma',gamma)
    for epoch in range(Epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            Y_ = torch.normal(0, sigma, size=(labels.shape)).to(
                device).double() + labels  # addeNoise(sigma, labels.detach().numpy()).double()
            # Y_ = addeNoise(sigma, labels.cpu().detach().numpy()).to(device).double()
            # setup_seed(i)
            if mode == 'norandom':
                Y_dot = None
            ...
            intermidiate_to_server=torch.sigmoid(intermidiate_to_server)
            loss_recon = gamma * BCE(intermidiate_to_server.view(-1, 1),
                                     Y_dot.view(-1, 1).type_as(intermidiate_to_server))
            ...
            outputs_test, intermidiate_to_server_test, _ = splitnn_GAFM(t[0].to(device).double(),
        ...
        if (grads and epoch == Epochs - 1):
            print('training_labels:', torch.transpose(training_labels[-1], 0, 1),
                  "\noutputs_list:" ,torch.transpose(outputs_list[-1], 0, 1),
                  "\n grad_gan:", grad_gan[-1],
                  "\n grad_recon:",grad_recon[-1]
                 )
            logger.write(
                '\ntraining_labels:{}\n outputs_list:{}\n grads:{}\n grad_gan:{}\n grad_recon:{} '.format(
                    torch.transpose(training_labels[-1], 0, 1), torch.transpose(outputs_list[-1], 0, 1), grads[-1],grad_gan[-1],grad_recon[-1]))
            logger.flush()
